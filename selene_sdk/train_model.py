...
class TrainModel(object):
    ...
    checkpoint_resume : str or None, optional
        ...
        using `torch.load`.
    metrics: dict
        Model diagnostics as a dictionary to track while training.
    write_weights: bool
        Whether to write first-layer convolutional filter weights to .txt
        file during training.
    filter_regularization: None or numeric
        Strength of sparse-group lasso (L1/L2) regularization applied to first-layer
        convolutional filters.
    weight_regularization: None or numeric
        Strength of sparsity (L1) regularization applied to all model weights.
    ...
    def _filter_regularization(self):
        wts = list(self.model.state_dict().values())[0]
        penalty = self.filter_regularization
        reg_loss = 0
        for param in wts:
            reg_loss += torch.sqrt(torch.sum(param**2))
        return penalty * reg_loss
        

    def _weight_regularization(self):
        wts=self.model.parameters()
        penalty = self.weight_regularization
        reg_loss = 0
        for param in wts:
            ...
            if step % self.nth_step_save_checkpoint == 0:
                ...
                if self.save_new_checkpoints is not None and \
                    ...
                    if self.write_weights:
                        checkpoint_filename = "model_weights-{0}".format(
                            strftime("%m%d%H%M%S"))
                        self._write_weights(
                            checkpoint_dict, False, filename=checkpoint_filename)
                        logger.debug("Saving weights `{0}.pth.tar`".format(
                            checkpoint_filename))
                else:
                    self._save_checkpoint(
    ...
    def train(self):
        ...
        if self.filter_regularization is not None:
            loss += self._filter_regularization()
    ...
    def _write_weights(self, state, is_best, filename="filter_weights", write_offsets=True):
        ...
        """
        logger.debug("[TRAIN] {0}: Saving weights state to file.".format(
            state["step"]))
        cp_filepath = os.path.join(
            self.output_dir, filename)
        mydict = state['state_dict']
        data = list(mydict.values())[0].cpu().numpy()
        with open(cp_filepath + '.txt' , 'w') as outfile:
            outfile.write('# Array shape: {0}\n'.format(data.shape))
            for data_slice in data:
                np.savetxt(outfile, data_slice, fmt='%-7.2f')
                outfile.write('# New slice\n')
        if is_best:
            best_filepath = os.path.join(self.output_dir, "best_model_" + filename)
            with open(best_filepath + '.txt' , 'w') as outfile:
                outfile.write('# Array shape: {0}\n'.format(data.shape))
                for data_slice in data:
                    np.savetxt(outfile, data_slice, fmt='%-7.2f')
                    outfile.write('# New slice\n')
            ...
            for mot in range(data.shape[0]):
                ...
                for row in range(ppm.shape[0]):
                    for nuc in range(4):
                        meme_file.write(" " + '{:.4f}'.format(round(ppm[row,nuc],4)))
                    meme_file.write("\n")
                meme_file.write("\n")    
            meme_file.close()
